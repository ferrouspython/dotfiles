...
static inline float angle_between(const float a_in[3], const float b_in[3]) {
    float a[3] = {a_in[0], a_in[1], a_in[2]};
    float b[3] = {b_in[0], b_in[1], b_in[2]};
    normalize3(a); normalize3(b);
    float c = clampf(a[0]*b[0] + a[1]*b[1] + a[2]*b[2], -1.0f, 1.0f);
    return acosf(c);
}

// Disk-intersection FOV test: returns true if a circular object with apparent angular radius 'obj_alpha'
// intersects a sensor cone of half-angle 'fov_half' about 'boresight'.
static bool in_fov_disk(const float boresight[3], float fov_half, const float dir_to_center[3], float obj_alpha) {
    float gamma = angle_between(boresight, dir_to_center);
    return gamma <= (fov_half + obj_alpha);
}
...
static void propagate_and_report(gs_adcs_config_t &conf,
    ...
    const float R_earth = 6378137.0f;        // [m]
    const float R_sun   = 695700e3f;         // [m]
    float r_sc = sqrtf(ephem.scRECI[0]*ephem.scRECI[0] + ephem.scRECI[1]*ephem.scRECI[1] + ephem.scRECI[2]*ephem.scRECI[2]);
    float d_sun = sqrtf((sunECI[0]-ephem.scRECI[0])*(sunECI[0]-ephem.scRECI[0]) +
                        (sunECI[1]-ephem.scRECI[1])*(sunECI[1]-ephem.scRECI[1]) +
                        (sunECI[2]-ephem.scRECI[2])*(sunECI[2]-ephem.scRECI[2]));
    float a_earth = (r_sc > R_earth) ? asinf(clampf(R_earth / r_sc, 0.0f, 1.0f)) : (float)M_PI_2;
    float a_sun   = asinf(clampf(R_sun / d_sun, 0.0f, 1.0f));

    bool earth_in_st = in_fov_disk(startracker_boresight, st_half, toEarthBody, a_earth);
    bool sun_in_st   = in_fov_disk(startracker_boresight, st_half, toSunBody,   a_sun);
    bool earth_in_pl = in_fov_disk(payload_boresight,     pl_half, toEarthBody, a_earth);
    bool sun_in_pl   = in_fov_disk(payload_boresight,     pl_half, toSunBody,   a_sun);
    ...
    if (stats) {
        stats->total++;
        if (in_eclipse) stats->eclipse++;
        if (earth_in_st) stats->st_earth_in++;
        if (sun_in_st)   stats->st_sun_in++;
        if (earth_in_pl) stats->pl_earth_in++;
        if (sun_in_pl)   stats->pl_sun_in++;
    }
}
...
int main(int argc, char** argv) {
    ...
    float fov_margin_deg = 10.0f; // additional margin for fov
    startracker_fov_deg += 2*fov_margin_deg; // add margin to startracker fov
    payload_fov_deg += 2*fov_margin_deg; // add margin to payload fov

    // 3) Define LVLH->body Euler offset (rad), 3-2-1 (X-Y-Z used by EulerToQuaternion in this codebase)
    float euler_offset[3] = {
        0.0 * M_PI / 180.0,     // X rotation
        -45.0 * M_PI / 180.0,   // Y rotation
    ...
    // 8) Summary report
    if (stats.total > 0) {
        auto pct = [](long x, long N) -> double { return (N > 0) ? (100.0 * (double)x / (double)N) : 0.0; };
        std::printf("==== Summary ====\n");
        std::printf("Euler Angles: [%.6f, %.6f, %.6f]\n", euler_offset[0], euler_offset[1], euler_offset[2]);
        std::printf("Samples:      %ld\n", stats.total);
        std::printf("Eclipse:      %ld (%.1f%%)\n", stats.eclipse, pct(stats.eclipse, stats.total));
        std::printf("StarTracker:  Earth IN %ld (%.1f%%), Sun IN %ld (%.1f%%)\n",
                    stats.st_earth_in, pct(stats.st_earth_in, stats.total),
                    stats.st_sun_in,   pct(stats.st_sun_in,   stats.total));
        std::printf("Payload:      Earth IN %ld (%.1f%%), Sun IN %ld (%.1f%%)\n\n",
                    stats.pl_earth_in, pct(stats.pl_earth_in, stats.total),
                    stats.pl_sun_in,   pct(stats.pl_sun_in,   stats.total));
    }
    return 0;
}
