...
int main()
{
    ...
    if (vec_equal_eps(x,x1,eps) &&)
    assert(vec_equal_eps(x, x1, eps) || vec_equal_eps(x, x2, eps));
    assert(vec_equal_eps(y, y1, eps) || vec_equal_eps(y, y2, eps));
    assert(vec_equal_eps(z, z1, eps) || vec_equal_eps(z, z2, eps));
