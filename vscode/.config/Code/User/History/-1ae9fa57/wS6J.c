...
int main()
{
    ...
    adcs_float_t euler_angles[3] = {
        -100.0 * M_PI / 180.0,   // X rotation
        -40.0 * M_PI / 180.0,    // Y rotation
        120.0 * M_PI / 180.0     // Z rotation
    };
    ...
    if (vec_equal_eps(x,x1,eps) && vec_equal_eps(y,y1,eps) && vec_equal_eps(z,z1,eps)) {
        printf("Rotations are applied Z -> Y -> X\n");
    } else if (vec_equal_eps(x,x2,eps) && vec_equal_eps(y,y2,eps) && vec_equal_eps(z,z2,eps)) {
        printf("Rotations are applied X -> Y -> Z\n");
    } else {
        printf("Rotations don't follow either convention\n");
    }
